...
#define ADC_Normalize( value ) 100-((value-menor)*100)/(mayor-menor)
uint8_t offset, var, menor, mayor, primera=1, segunda = 1;
...
uint8_t ADC_Read( uint8_t channel )
{
	...
	if(channel > 7)
	{
		DIDR2 |=1<<(channel-8);		//desactiva la parte digital 
		DDRK &= ~(1<<(channel-8));  //pcx como salida
		ADMUX = (1<<REFS0)|((channel-8)<<MUX0);
		ADCSRB |=1<<MUX5;				//parte de medicion a tierra
	}
	else
	{
		DIDR0 |=1<<channel;		//desactiva la parte digital
		DDRA &= ~(1<<channel);  //pcx como salida
